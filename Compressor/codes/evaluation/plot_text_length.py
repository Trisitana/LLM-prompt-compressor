...
methods = {
    "ICAE": "ICAE_512to1_15epoch",
    "lora_param": "lora_param_512to1_15_epoch"
}
...
for method_name in methods.keys():
    ...
    plt.plot(text_lengths, metrics["bleu"][method_name],
    ...
    for x, y in zip(text_lengths, metrics["bleu"][method_name]):
        plt.annotate(f'{y:.3f}', 
                    (x, y),
...
plt.xlabel('Abstract Length', fontsize=12)
plt.ylabel('BLEU Score', fontsize=12)
# plt.title('BLEU Score vs Text Length (512→2)', fontsize=14)
...
plt.tight_layout()
plt.savefig('text_length_bleu_comparison_1.png', dpi=300, bbox_inches='tight')
plt.close()

# Print the data for verification
print("\nBLEU Scores by Text Length:")
for method_name in methods.keys():
    print(f"\n{method_name}:")
    for length, score in zip(text_lengths, metrics["bleu"][method_name]):
