...
def main(ref_file, cand_file, output_file):
    with open(ref_file, 'r', encoding='utf-8') as ref_f, \
         open(cand_file, 'r', encoding='utf-8') as cand_f, \
         open(os.path.join(os.path.dirname(ref_file), 'context'), 'r', encoding='utf-8') as ctx_f:
        references = ref_f.readlines()
        candidates = cand_f.readlines()
        contexts = ctx_f.readlines()

    results = []
    i = 1
    for reference, candidate, context in zip(references, candidates, contexts):
        ...
        result['context_tokens'] = len(tokenizer.encode(context.strip()))
...
if __name__ == "__main__":
    root = "codes/evaluation/512QA_qwen_raw"
    num = '8'
    # additional information like step
    ref_file = os.path.join(root, num, 'tar_results')
    cand_file = os.path.join(root, num, 'gen_results')
    ...
    averages.to_csv(average_output, header=True) 