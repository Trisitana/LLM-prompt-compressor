...
from transformers import AutoTokenizer, LlamaForCausalLM, TrainingArguments, Trainer, AutoModelForCausalLM, pipeline
import os
...
if __name__ == "__main__":
    ...
    text_file_path = os.path.join("select_context/selective_0.0078125_train_new.jsonl")
    

    # root path to save results
    root = "select_pipeline/0.0078125"
    ...
    for batch_texts in data_loader:
        ...
        # Extract the generated answer by removing the prompt
        generated_text = response[len(full_prompt):].strip()
        generated_token_length = len(chat_pipeline.tokenizer.encode(generated_text))
